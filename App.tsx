...
const App: React.FC = () => {
  ...
  const handleFileSelect = async (base64: string, mimeType: string) => {
    // Store full data URI for display
    const fullImageSrc = `data:${mimeType};base64,${base64}`;
    setState(prev => ({ ...prev, status: 'ANALYZING', imageSrc: fullImageSrc, error: null }));
    
    try {
      const result = await analyzeImage(base64, mimeType);
      setState(prev => ({ ...prev, status: 'RESULT', result }));
    } catch (error: any) {
      setState(prev => ({ 
        ...prev, 
        status: 'ERROR', 
        error: error.message || 'Something went wrong. The AI might be overwhelmed by your aura.' 
      }));
    }
  };
    ...
    <div className="min-h-screen bg-[#050505] text-white flex flex-col font-sans selection:bg-neon/30">
            ...
            <div className="text-center space-y-8 sm:space-y-12 animate-in fade-in zoom-in duration-500">
              <div className="space-y-3 sm:space-y-4">
                <h1 className="flex flex-wrap items-center justify-center gap-4 text-4xl sm:text-6xl md:text-8xl font-black tracking-tighter leading-tight pb-2">
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-yellow-600 drop-shadow-[0_0_15px_rgba(234,179,8,0.3)] py-2 px-4">CHAD</span>
                  <span className="text-white text-xl sm:text-3xl font-light tracking-widest opacity-60">OR</span>
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-stone-500 to-stone-700 drop-shadow-[0_0_15px_rgba(120,113,108,0.3)] py-2 px-4">CHUD</span>
                </h1>
                <p className="text-base sm:text-xl text-gray-400 max-w-2xl mx-auto px-2 leading-relaxed">
                  Let AI judge your physiognomy. Will you mog the competition or is it over for you? Upload a photo for a brutally honest assessment.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 text-left max-w-3xl mx-auto pt-4 sm:pt-8">
                {[
                  { title: "Physiognomy Analysis", desc: "Precise evaluation of facial harmony, grooming standards, and genetic potential." },
                  { title: "Looksmaxxing Protocol", desc: "Curated, high-impact product recommendations to fix flaws and maximize your stats." }
                ].map((feature, idx) => (
      ...
      <footer className="w-full py-4 sm:py-6 border-t border-white/10 bg-black text-center">
        <p className="text-gray-600 text-xs sm:text-sm px-4">
          ChadOrChud © {new Date().getFullYear()}. Results generated by AI for entertainment.
        </p>
      </footer>
    </div>
  );
